...
public partial class WKF_Leave_ListHR : Ede.Uof.Utility.Page.BasePage
{
    ...
    protected void gvLeave_RowDataBound(object sender, GridViewRowEventArgs e)
    {
        ...
        if (e.Row.RowType == DataControlRowType.DataRow)
        {
            ...
            string dataKey = gvLeave.DataKeys[e.Row.RowIndex].Values["LYV"].ToString();
    ...
    protected void gvLeave_SelectedIndexChanging(object sender, GridViewSelectEventArgs e)
    {
        ...
        ExpandoObject param = new { LYV = key, SiteName = hfSiteName.Value, Account = Current.Account }.ToExpando();
        if (key == "")
        {
        ...
        {
            Dialog.Open2(Approve, "~/CDS/LYV/Plugin/Leave/Leave_Modal.aspx", "Approve All", 600, 300, Dialog.PostBackType.AfterReturn, param);
        }
    }
    ...
    private void LoadDataLeave()
    {
        ...
        DataTable dt = uco.GetListLeave(D_STEP_DESC, flowflag, DV_MA_, LYV, LeaverID, StartDate, EndDate, Documents, Factory, Account);
    ...
    public void OnOK_Click(object sender, EventArgs e)
    {
        ...
        {
            string[] dsLYV = key.Split(",");
            string where = "LYV in ('" + dsLYV[0] + "'";
            if (dsLYV.Length > 1)
            {
                for (int i = 1; i < dsLYV.Length; i++)
                {
                    where += ",'" + dsLYV[i] + "'";
                }
            }
