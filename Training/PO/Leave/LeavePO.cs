...
namespace LYV.Leave.PO
{
    internal class LeavePO : Ede.Uof.Utility.Data.BasePersistentObject
    {
        internal string getFlowflag(string LYV)
        {
            ...
            string cmdflowflag = @"SELECT flowflag FROM LYV_Leave WHERE LYV = @LYV";
        ...
        internal string CheckLeaveDate(string USERID, string StartDate, string EndDate, string type)
        {
            ...
            string cmdTxt = @"SELECT * FROM LYV_Leave WHERE LeaverID = @USERID AND (StartDate BETWEEN @StartDate AND @EndDate OR EndDate BETWEEN @StartDate AND @EndDate) AND flowflag <> 'X' ";
        ...
        //    }
        //}
        //internal string Check_KhoaSo_All(string LYV)
        //{
        //    string conn = Training.Properties.Settings.Default.UOF.ToString();
        //    this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn);
        //    string cmdTxt = @"SELECT LeaverID, StartDate FROM LYV_Leave WHERE LYV = @LYV ";
        ...
        //    }
        //}
        internal void InsertLeaveFormData(string LYV, string UserID, string Factory, XElement xE)
        {
            string conn = Training.Properties.Settings.Default.UOF.ToString();
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn);
            ...
            string cmdTxt = @"  INSERT INTO LYV_Leave
                                (	 [LYV] ,  
                                     [Factory] , 
                                     [DepartmentID] ,
                                     [LeaverID] ,
        ...
        internal void DisableFlowflag(string LYV)
        {
            string conn1 = Training.Properties.Settings.Default.UOF.ToString();
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn1);

            string cmdTxt = @"SELECT LeaverID, StartDate, EndDate, Type FROM LYV_Leave WHERE LYV = @LYV AND flowflag IN ('Z')";

            DataTable dt = new DataTable();
            this.m_db.AddParameter("@LYV", LYV);
            dt.Load(this.m_db.ExecuteReader(cmdTxt));

            string cmdTxt1 = @"UPDATE LYV_Leave SET flowflag = 'X' WHERE LYV = '" + LYV + "'";
        ...
        //internal string CheckHRMData(string LYV)
        //{
        //    string conn1 = Training.Properties.Settings.Default.UOF.ToString();
        //    this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn1);

        //    string cmdTxt = @"SELECT LeaverID, StartDate, EndDate FROM LYV_Leave WHERE LYV = @LYV AND flowflag IN ('N','P')";
        ...
        //    }
        //}
        internal void UpdateFormStatus(string LYV, string SiteCode, string signStatus, string Factory, XElement xE)
        {
            string conn = Training.Properties.Settings.Default.UOF.ToString();
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn);
            ...
            if ((flowflag == "NP" || flowflag == "N") && SiteCode != "ReturnToApplicant")
            {
                ...
                string cmdTxt = @"  UPDATE LYV_Leave SET
                                     [Factory] = @Factory,
                ...
                {
                    string cmdTxt1 = "UPDATE LYV_Leave SET flowflag = 'P' WHERE LYV = @LYV ";
                    this.m_db.AddParameter("@LYV", LYV);
                    this.m_db.ExecuteNonQuery(cmdTxt1);
                }
            }
            ...
            {
                string cmdTxt = "UPDATE LYV_Leave SET flowflag = 'NP' WHERE LYV = @LYV ";
                this.m_db.AddParameter("@LYV", LYV);
                this.m_db.ExecuteNonQuery(cmdTxt);
            }
            else if ((result == "1" && SiteCode == "SE1" && signStatus == "Approve") || (result == "2" && SiteCode == "SE2" && signStatus == "Approve"))
            {
                string cmdTxt = @"UPDATE LYV_Leave SET flowflag='Z' WHERE LYV = @LYV AND flowflag IN ('N','P')";
                this.m_db.AddParameter("@LYV", LYV);
                this.m_db.ExecuteNonQuery(cmdTxt);
            }
            else if (signStatus == "Disapprove")
            {
                string cmdTxt = @"UPDATE LYV_Leave SET flowflag='X' WHERE LYV = @LYV ";
                this.m_db.AddParameter("@LYV", LYV);
                this.m_db.ExecuteNonQuery(cmdTxt);
            }
        ...
        internal DataTable GetListLeave(string D_STEP_DESC, string flowflag, string DV_MA_, string LYV, string LeaverID, string StartDate, string EndDate, string Documents, string Factory, string Account)
        {
            string conn = Training.Properties.Settings.Default.UOF.ToString();
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn);
            if (Factory != "")
            {
                ...
                string SQL = @" WITH TB_WKF_TASK_NODE_CTE AS (
                                ...
                                SELECT LYV_Leave.LYV, LYV_Leave.USERID, Name_CTE.NAME as USERNAME, LYV_Leave.DepartmentID DV_MA_, LYV_Leave.DepartmentID, LYV_Leave.LeaverID, LYV_Leave.LeaverName, LYV_Leave.Type,
                                       LYV_Leave.TotalDay, LYV_Leave.StartDate, LYV_Leave.EndDate, LYV_Leave.DHMonth, LYV_Leave.YDLeave, LYV_Leave.Total, LYV_Leave.Reason, LYV_Leave.Remark, 
	                                   CONVERT(VARCHAR, LYV_Leave.USERDATE, 120) USERDATE, ISNULL(TB_WKF_TASK_NODE_CTE.STEP_DESC, '已結案') AS D_STEP_DESC, TB_WKF_TASK.TASK_ID, TB_WKF_TASK.ATTACH_ID, LYV_Leave.Documents 
                                FROM LYV_Leave LEFT JOIN TB_WKF_TASK ON LYV_Leave.LYV = TB_WKF_TASK.DOC_NBR 
                                LEFT JOIN (SELECT * FROM TB_WKF_TASK_NODE_CTE WHERE rn = 1) AS TB_WKF_TASK_NODE_CTE ON LYV_Leave.LYV = TB_WKF_TASK_NODE_CTE.DOC_NBR 
                                LEFT JOIN Name_CTE ON Name_CTE.UserID = LYV_Leave.USERID 
                                WHERE 1=1 " + where + @" 
                                ORDER BY LYV_Leave.LYV desc ";
            ...
            else
            {
                ...
                string SQL = @" SELECT LYV_Leave.LYV, LYV_Leave.USERID, Name.NAME as USERNAME, LYV_Leave.DepartmentID DV_MA_, LYV_Leave.DepartmentID, LYV_Leave.LeaverID, LYV_Leave.LeaverName, LYV_Leave.Type, 
                                       LYV_Leave.TotalDay, LYV_Leave.StartDate, LYV_Leave.EndDate, LYV_Leave.DHMonth, LYV_Leave.YDLeave, LYV_Leave.Total, LYV_Leave.Reason, LYV_Leave.Remark, 
                                       CONVERT(VARCHAR,LYV_Leave.USERDATE,120) USERDATE, '已結案' AS D_STEP_DESC, TB_WKF_TASK.TASK_ID, TB_WKF_TASK.ATTACH_ID, LYV_Leave.Documents 
                                FROM LYV_Leave LEFT JOIN TB_WKF_TASK on LYV_Leave.LYV=TB_WKF_TASK.DOC_NBR 
                                LEFT JOIN ( SELECT SUBSTRING(ACCOUNT, PATINDEX('%[0-9]%', ACCOUNT), LEN(ACCOUNT)) UserID, NAME FROM TB_EB_USER ) AS Name ON Name.UserID = LYV_Leave.USERID 
                                WHERE 1=1 " + where + @" 
                                ORDER BY LYV_Leave.LYV desc ";
        ...
        internal void UpdateWFLeave(string flag, string Where)
        {
            string conn1 = Training.Properties.Settings.Default.UOF.ToString();
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn1);
            string cmdTxt = "UPDATE LYV_Leave SET Documents = '" + flag + "' WHERE " + Where;
            this.m_db.ExecuteNonQuery(cmdTxt);
