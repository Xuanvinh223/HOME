...
namespace LYV.Acceptance.PO
{
    internal class AcceptancePO : Ede.Uof.Utility.Data.BasePersistentObject
    {
        private string connUOF = Training.Properties.Settings.Default.UOF.ToString();
        ...
        {
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(connUOF);
            ...
            string cmdTxt = @"  INSERT INTO [dbo].[LYV_Acceptance]
                                   ([LYV]
                                   ,[ListType]
                                   ,[PurchaseRequestNo]
                                   ,[AcceptanceDate]
        ...
        {
            string cmdflowflag = @"SELECT flowflag, ListType FROM LYV_Acceptance WHERE LYV = @LYV";
            ...
            if (flowflag == "NP" || flowflag == "N" && SiteCode != "ReturnToApplicant")
            {
                ...
                string RKNO = xE.Attribute("RKNO").Value;
                string LType = ListType.Substring(0, ListType.IndexOf('@'));
                string cmdTxt = @" UPDATE [dbo].[LYV_Acceptance]
                                SET
                                    [ListType] = @ListType,                          
            ...
            {
                this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn);
                string cmdTxt = "UPDATE LYV_Acceptance SET flowflag = 'NP' WHERE LYV = @LYV ";
                this.m_db.AddParameter("@LYV", LYV);
                this.m_db.ExecuteNonQuery(cmdTxt);
                this.m_db.Dispose();
            }
        ...
        internal void UpdateFormResult(string LYV, string formResult)
        {
            if (formResult == "Adopt")
            {
                ...
                string cmdTxt = @"UPDATE LYV_Acceptance SET flowflag='Z' WHERE LYV = @LYV AND flowflag IN ('N','P')";
                this.m_db.AddParameter("@LYV", LYV);
                this.m_db.ExecuteNonQuery(cmdTxt);
            }
            else if (formResult == "Reject" || formResult == "Cancel")
            {
                string conn1 = Training.Properties.Settings.Default.UOF.ToString();
                this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn1);
                string cmdTxt = @"UPDATE LYV_Acceptance SET flowflag='X' WHERE LYV = @LYV AND flowflag IN ('N','P')";
                this.m_db.AddParameter("@LYV", LYV);
                this.m_db.ExecuteNonQuery(cmdTxt);
            }
            else
            {
                string conn1 = Training.Properties.Settings.Default.UOF.ToString();
                this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn1);
                string cmdTxt = @"UPDATE LYV_Acceptance SET flowflag='NP' WHERE LYV = @LYV AND flowflag IN ('N','P')";
                this.m_db.AddParameter("@LYV", LYV);
                this.m_db.ExecuteNonQuery(cmdTxt);
            }
        }
        ...
        internal string getFlowflag(string LYV)
        {
            ...
            string conn = Training.Properties.Settings.Default.UOF.ToString();
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(conn);
            string cmdflowflag = @"SELECT flowflag FROM LYV_Acceptance WHERE LYV = @LYV";
        ...
        internal DataTable GetGridView_Close(string LYV, string RKNO, string ListType, string Sdate, string Edate)
        {

            string connStr = Training.Properties.Settings.Default.UOF.ToString();
            this.m_db = new Ede.Uof.Utility.Data.DatabaseHelper(connStr);
            string cmdTxt = @"select LYV, ListType, PurchaseRequestNo, AcceptanceDate, RKNO, Department, Applicant, Description, PropertyNumbers, UserID, UserDate, TB_WKF_TASK.TASK_ID   from LYV_Acceptance 
                            LEFT JOIN TB_WKF_TASK ON TB_WKF_TASK.DOC_NBR = LYV_Acceptance.LYV
                            where LYV like '%" + LYV + @"%' and  RKNO like '%" + RKNO + @"%' and ListType like N'%" + ListType + @"%'";
            if (Sdate != "")
            {
